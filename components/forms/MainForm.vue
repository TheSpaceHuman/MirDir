    ...
    <section class="main-form">
      ...
      <form @submit.prevent="send">
        <a-row type="flex" :gutter="15">
          <template v-if="col === 1">
            <a-col :xs="24" :sm="24" :md="24" class="mb-15" v-for="field in fields" :key="field.name">
          ...
          <template v-else>
            <a-col :xs="24" :sm="24" :md="12" class="mb-15" v-for="field in fields" :key="field.name">
              <label :for="field.name" class="form-label" v-if="field.label">{{field.label}} <span class="color--danger" v-if="field.required">*</span></label>
              <a-input :placeholder="field.placeholder" :type="field.text" :id="field.name" v-model="form[field.name]" />
            </a-col>
          </template>
          <a-col :xs="24" :sm="24" :md="24" class="mb-15" v-if="personal.active">
